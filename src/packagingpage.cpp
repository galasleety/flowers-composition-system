...
PackagingPage::PackagingPage(QWidget *parent) : QWidget(parent), m_isEditing(false), m_isAddingNew(false) {
    setStyleSheet("background-color: #FFFFFF;");
    setupUI();
    loadPackaging();
}

void PackagingPage::setupUI() {
    ...
    QLabel *titleLabel = new QLabel("КАТАЛОГ УПАКОВКИ", leftPanel);
    ...
    connect(m_searchEdit, &QLineEdit::textChanged, this, &PackagingPage::onSearchTextChanged);
    leftLayout->addWidget(m_searchEdit);

    m_packagingList = new QListWidget(leftPanel);
    m_packagingList->setFrameShape(QFrame::NoFrame);
    m_packagingList->setFocusPolicy(Qt::NoFocus);
    m_packagingList->setIconSize(QSize(30, 30));
    m_packagingList->setStyleSheet(
    ...
    connect(m_packagingList, &QListWidget::itemSelectionChanged, this, &PackagingPage::onPackagingSelected);
    leftLayout->addWidget(m_packagingList);

    m_btnAddPackaging = new QPushButton("+ Добавить упаковку", leftPanel);
    m_btnAddPackaging->setFixedHeight(38);
    m_btnAddPackaging->setCursor(Qt::PointingHandCursor);
    m_btnAddPackaging->setStyleSheet("QPushButton { border: 1px solid #DCDCDC; border-radius: 12px; font-family: 'Century Gothic'; font-size: 12px; color: #555; } QPushButton:hover { background: #F5F5F5; }");
    connect(m_btnAddPackaging, &QPushButton::clicked, this, &PackagingPage::onAddPackagingClicked);
    leftLayout->addWidget(m_btnAddPackaging);
    ...
    m_nameLabel = new QLabel("Название упаковки", vW);
    ...
    m_nameEditField->setPlaceholderText("Название упаковки...");
    ...
    m_descEditField->setPlaceholderText("Описание упаковки...");
    ...
    m_btnEditPackaging = new QPushButton("Редактировать", m_infoCard);
    m_btnEditPackaging->setFixedSize(125, 36);
    m_btnEditPackaging->setCursor(Qt::PointingHandCursor);
    m_btnEditPackaging->setStyleSheet("QPushButton { border: 2px solid #DCDCDC; border-radius: 18px; font-family: 'Century Gothic'; font-size: 12px; } QPushButton:hover { background: #F5F5F5; }");
    connect(m_btnEditPackaging, &QPushButton::clicked, this, &PackagingPage::onEditPackagingClicked);

    m_btnDeletePackaging = new QPushButton("Удалить", m_infoCard);
    m_btnDeletePackaging->setFixedSize(60, 36);
    m_btnDeletePackaging->setCursor(Qt::PointingHandCursor);
    m_btnDeletePackaging->setStyleSheet("QPushButton { border: none; color: #D86B7A; font-family: 'Century Gothic'; font-size: 12px; } QPushButton:hover { text-decoration: underline; }");
    connect(m_btnDeletePackaging, &QPushButton::clicked, this, &PackagingPage::onDeletePackagingClicked);

    btnContainer->addWidget(m_btnEditPackaging);
    btnContainer->addWidget(m_btnDeletePackaging, 0, Qt::AlignCenter);
    ...
    // --- 2. СЕТКА ХАРАКТЕРИСТИК (2x2 для Упаковки) ---
    ...
    addViewStat("Материал", m_valMaterial, 0, 0);
    addViewStat("Водонепроницаемость", m_valWaterproof, 0, 1);
    addViewStat("Воздухопроницаемость", m_valBreathability, 1, 0);
    addViewStat("Амортизация", m_valShockAbs, 1, 1);
    ...
    m_editMaterial = new QComboBox();
    m_editMaterial->addItems({"Крафт-бумага", "Пленка", "Фетр", "Сетка", "Фоамиран", "Сизаль", "Органза", "Картон"});

    m_editWaterproof = new QComboBox();
    m_editWaterproof->addItem("Нет", 0);
    m_editWaterproof->addItem("Да", 1);

    m_editBreathability = new QComboBox();
    m_editBreathability->addItems({"Нет", "Низкая", "Средняя", "Высокая"});

    m_editShockAbs = new QComboBox();
    m_editShockAbs->addItems({"Нет", "Низкая", "Средняя", "Высокая"});

    addEditStatCombo("Материал", m_editMaterial, 0, 0);
    addEditStatCombo("Водонепроницаемость", m_editWaterproof, 0, 1);
    addEditStatCombo("Воздухопроницаемость", m_editBreathability, 1, 0);
    addEditStatCombo("Амортизация", m_editShockAbs, 1, 1);
    ...
    m_varTitle = new QLabel("ВАРИАЦИИ УПАКОВКИ", scrollContent);
    ...
    connect(m_btnAddVariation, &QPushButton::clicked, this, &PackagingPage::onAddVariationClicked);
...
void PackagingPage::loadPackaging() {
    m_packagingList->clear();
    QSqlDatabase db = QSqlDatabase::database();
    if (!db.isOpen()) return;

    QSqlQuery q(db);
    if (q.exec("SELECT item_id, name FROM Items WHERE item_type = 'packaging' ORDER BY name")) {
        while (q.next()) {
            QListWidgetItem *item = new QListWidgetItem("   " + q.value(1).toString());
            item->setData(Qt::UserRole, q.value(0).toInt());
            // Если есть отдельная иконка для упаковки, поменяй тут:
            item->setIcon(QIcon(":/sources/icons/flower_icon.png"));
            item->setFont(QFont("Century Gothic", 11));
            m_packagingList->addItem(item);
        }
    }
}
...
void PackagingPage::onPackagingSelected() {
    QListWidgetItem *item = m_packagingList->currentItem();
    if (!item) {
    ...
    m_btnEditPackaging->setText("Редактировать");
    m_btnEditPackaging->setStyleSheet("QPushButton { border: 2px solid #DCDCDC; border-radius: 18px; font-family: 'Century Gothic'; font-size: 12px; color: #333; } QPushButton:hover { background: #F5F5F5; }");

    int id = item->data(Qt::UserRole).toInt();
    QSqlQuery q;

    // Джойним с таблицей Packaging_details
    q.prepare("SELECT i.name, i.description, "
              "(SELECT image FROM Item_variation WHERE item_id = i.item_id LIMIT 1), "
              "pd.material, pd.is_waterproof, pd.breathability, pd.shock_absorption "
              "FROM Items i LEFT JOIN Packaging_details pd ON i.item_id = pd.item_id "
              "WHERE i.item_id = :id");
    q.bindValue(":id", id);
    ...
    if (q.exec() && q.next()) {
        ...
        QString material = q.value(3).toString();
        int isWaterproof = q.value(4).toInt();
        QString breathability = q.value(5).toString();
        QString shockAbs = q.value(6).toString();

        m_valMaterial->setText(material.isEmpty() ? "-" : material);
        m_valWaterproof->setText(isWaterproof == 1 ? "Да" : "Нет");
        m_valBreathability->setText(breathability.isEmpty() ? "-" : breathability);
        m_valShockAbs->setText(shockAbs.isEmpty() ? "-" : shockAbs);

        m_editMaterial->setCurrentText(material);
        m_editWaterproof->setCurrentIndex(isWaterproof == 1 ? 1 : 0);
        m_editBreathability->setCurrentText(breathability);
        m_editShockAbs->setCurrentText(shockAbs);
...
void PackagingPage::onAddPackagingClicked() {
    m_packagingList->clearSelection();
    m_isEditing = true;
    m_isAddingNew = true;
    ...
    m_editMaterial->setCurrentIndex(0);
    m_editWaterproof->setCurrentIndex(0);
    m_editBreathability->setCurrentIndex(0);
    m_editShockAbs->setCurrentIndex(0);
    ...
    m_btnEditPackaging->setText("Сохранить");
    m_btnEditPackaging->setStyleSheet("QPushButton { border: none; border-radius: 18px; color: white; background: #D86B7A; font-family: 'Century Gothic'; font-weight: bold; }");
    m_btnDeletePackaging->hide();
}

void PackagingPage::onEditPackagingClicked() {
    if (!m_isEditing) {
        m_isEditing = true;
        m_isAddingNew = false;
        m_textStack->setCurrentIndex(1);
        ...
        if (newName.isEmpty() || newDesc.isEmpty()) {
            ...
            ErrorDialog err("Ошибка", "Заполните название и описание упаковки!", this->window());
        ...
        if (m_isAddingNew) {
            QSqlQuery qItems(db);
            // item_type теперь 'packaging'
            qItems.prepare("INSERT INTO Items (name, description, item_type) VALUES (:n, :d, 'packaging')");
            ...
            if (qItems.exec()) {
                ...
                qDetails.prepare("INSERT INTO Packaging_details (item_id, material, is_waterproof, breathability, shock_absorption) "
                                 "VALUES (:id, :mat, :water, :breath, :shock)");
                qDetails.bindValue(":id", newId);
                qDetails.bindValue(":mat", m_editMaterial->currentText());
                qDetails.bindValue(":water", m_editWaterproof->currentData().toInt());
                qDetails.bindValue(":breath", m_editBreathability->currentText());
                qDetails.bindValue(":shock", m_editShockAbs->currentText());
                qDetails.exec();
                ...
                loadPackaging();
                for(int i = 0; i < m_packagingList->count(); ++i) {
                    if (m_packagingList->item(i)->data(Qt::UserRole).toInt() == newId) {
                        m_packagingList->setCurrentRow(i);
                        break;
                    }
                }
            } else {
            ...
            QSqlQuery qDetails(db);
            qDetails.prepare("UPDATE Packaging_details SET material = :mat, is_waterproof = :water, "
                             "breathability = :breath, shock_absorption = :shock WHERE item_id = :id");
            qDetails.bindValue(":mat", m_editMaterial->currentText());
            qDetails.bindValue(":water", m_editWaterproof->currentData().toInt());
            qDetails.bindValue(":breath", m_editBreathability->currentText());
            qDetails.bindValue(":shock", m_editShockAbs->currentText());
...
void PackagingPage::renderPendingVariations() {
    QLayoutItem *child;
...
void PackagingPage::loadVariations(int itemId) {
    QLayoutItem *child;
    while ((child = m_variationsLayout->takeAt(0)) != nullptr) {
        delete child->widget();
        delete child;
...
void PackagingPage::onAddVariationClicked() {
    int id = -1;
...
void PackagingPage::onDeleteVariationClicked(int variationId) {
    QWidget *overlay = new QWidget(this->window());
    overlay->setStyleSheet("background-color: rgba(0, 0, 0, 150);");
    overlay->resize(this->window()->size());
    overlay->move(0, 0);
    ...
    ConfirmDialog dialog("Удаление", "Удалить эту вариацию упаковки?", this->window());
    dialog.move(this->window()->frameGeometry().center() - dialog.rect().center());
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        if (q.exec()) {
            int currentItemId = m_packagingList->currentItem()->data(Qt::UserRole).toInt();
...
void PackagingPage::onDeletePackagingClicked() {
    QListWidgetItem *item = m_packagingList->currentItem();
    if (!item) return;
    ...
    ConfirmDialog dialog("Удаление", "Удалить эту упаковку из каталога навсегда?", this->window());
    dialog.move(this->window()->frameGeometry().center() - dialog.rect().center());
    ...
    if (dialog.exec() == QDialog::Accepted) {
        ...
        q.bindValue(":id", id);
        if (q.exec()) loadPackaging();
    }
    overlay->deleteLater();
}

void PackagingPage::onSearchTextChanged(const QString &text) {
    for(int i = 0; i < m_packagingList->count(); ++i) {
        m_packagingList->item(i)->setHidden(!m_packagingList->item(i)->text().contains(text, Qt::CaseInsensitive));
    }
}